...
function LoginContent() {
  ...
  // Safely get origin only after mounting on the client
  useEffect(() => {
  ...
  const signInWithGoogle = async () => {
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        // Fallback to empty string if origin isn't set yet to avoid build errors
...
export default function LoginPage() {
  return (
    <div className="flex flex-col min-h-screen items-center justify-center bg-slate-50 px-4">
      ...
      <Suspense fallback={<div className="p-8 bg-white rounded-xl shadow-md">Chargement...</div>}>
        <LoginContent />
      </Suspense>
